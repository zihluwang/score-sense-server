...
public class ExamService {
    ...
    public ExamService(ExamRepository examRepository,
                       ...
                       @Qualifier("examIdCreator") GuidCreator<Long> examIdCreator, VacancyService vacancyService, QuestionService questionService, OptionService optionService, AnswerService answerService, ExamResultService examResultService) {
     ...
     *
     * @param examName 考试名称
     * @return 如果已存在，返回 {@code true}；否则返回 {@code false}
     */
    public boolean isExamNameExisted(String examName) {
        return examRepository.selectCountByCondition(Exam.EXAM.NAME.eq(examName)) != 0;
    }
    ...
    public Exam createExam(CreateExamRequest request) {
        var canCreate = isExamNameExisted(request.name());

        if (!canCreate) {
            throw new BaseBizException(HttpStatus.CONFLICT, "考试名称已存在");
        }

        var examId = examIdCreator.nextId();
     ...
     * 更新考试信息。
     *
     * @param request 更新考试信息请求
     * @return 被更新的考试
     */
    public Exam updateExam(UpdateExamRequest request) {
        if (isExamNameExisted(request.name())) {
            throw new BaseBizException(HttpStatus.CONFLICT, "考试已创建");
        }

        var exam = examRepository.selectOneById(request.id());
        Optional.ofNullable(request.name())
                .filter((_name) -> !_name.isBlank())
                .ifPresent(exam::setName);

        Optional.ofNullable(request.type())
                .ifPresent(exam::setType);

        Optional.ofNullable(request.description())
                .filter((_description) -> !_description.isBlank())
                .ifPresent(exam::setDescription);

        Optional.ofNullable(request.province())
                .filter((_province) -> !_province.isBlank())
                .ifPresent(exam::setProvince);

        Optional.ofNullable(request.prefecture())
                .filter((_prefecture) -> !_prefecture.isBlank())
                .ifPresent(exam::setPrefecture);
    ...
    public int deleteExam(Long examId) {
        // 删除岗位
        var deleteExamVacancyTask = CompletableFuture.runAsync(() -> vacancyService.deleteExamVacanciesByExamId(examId),
                ConcurrentConfig.CACHED_EXECUTORS);
        // 删除试题
        var deleteQuestionsTask = CompletableFuture.runAsync(() -> questionService.deleteByExamId(examId),
                ConcurrentConfig.CACHED_EXECUTORS);
        // 删除选项
        var deleteOptionsTask = CompletableFuture.runAsync(() -> optionService.deleteByExamId(examId),
                ConcurrentConfig.CACHED_EXECUTORS);
        // 删除答案
        var deleteAnswersTask = CompletableFuture.runAsync(() -> answerService.deleteByExamId(examId),
                ConcurrentConfig.CACHED_EXECUTORS);
        // 删除考试结果
        var deleteExamResultsTask = CompletableFuture.runAsync(() -> examResultService.deleteByExamId(examId),
                ConcurrentConfig.CACHED_EXECUTORS);
        // 删除考试
        var deleteExamsTask = CompletableFuture.supplyAsync(() -> examRepository.deleteById(examId),
                ConcurrentConfig.CACHED_EXECUTORS);
        CompletableFuture.allOf(deleteExamVacancyTask, deleteQuestionsTask, deleteOptionsTask, deleteAnswersTask, deleteExamResultsTask, deleteExamsTask)
                .join();
        // 最后删除考试
        return deleteExamsTask.getNow(0);
    }
}
